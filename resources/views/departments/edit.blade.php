...
<div class="container py-4" style="max-width:580px;">
    <div class="mb-4">
        ...
        <h1 class="h3 mt-2 mb-1 text-scd-primary fw-bold">Editar Departamento</h1>
        <p class="text-muted mb-0">A alterar: <strong class="text-scd-primary">{{ $department->name }}</strong></p>
    </div>
    @if($errors->any())
            ...
            <form action="{{ route('departments.update', $department) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $department->name) }}"
                           class="form-control bg-scd-surface text-scd-primary border-secondary @error('name') is-invalid @enderror" required>
                ...
                <div class="mb-4">
                    ...
                    <select id="manager_id" name="manager_id"
                        ...
                        @foreach($managers as $m)
                            <option value="{{ $m->id }}"
                                {{ old('manager_id', $department->manager_id) == $m->id ? 'selected' : '' }}>
                                {{ $m->name }} ({{ $m->email }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="d-flex flex-column flex-sm-row gap-2 gap-sm-3 mt-2">
                    <button type="submit" class="btn btn-primary px-5"><i class="bi bi-save2 me-2"></i>Guardar</button>
